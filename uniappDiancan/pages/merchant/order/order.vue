<template>
	<view class="takeoutOrder flex">
		<u-navbar title="门店订单" leftIconSize="0" :placeholder="true" :fixed="true" :autoBack="false">
		</u-navbar>




		<view class="daytype flex ac jsb">
			<view class="flex ac">
				...
				<view class="daytypeli" :class="!sxtime&&current==5?'daytypelic':''" @click.stop="tabbutclick(5)">
					已取消
				</view>
			</view>
			<view class="">
		...
		<view class="tabsbut flex  ac" v-if="current<5">
		...
		<view class="flex orderlist f1" style="position: relative;">
			<scroll-view style="width: 100%;height: 100%;" scroll-y="true" @scrolltolower="scrolltolower">
				...
				<view class="pad" v-else>
					...
					<view class="content_item flexs flexc"
						...
						<view class="" style="padding-top: 16rpx;box-sizing: border-box;">
							...
							<view class="" v-if="item.dc_type==2">
								<view class="content_Bom flex je ac">
									<template v-if="item.status==1">
										...
										<view class="btm4" @click.stop="cashPay(item.order_no,item)">
											现金支付
										</view>
										<view class="btm3"
											@click.stop="addAdish(item.id,item.seat_id,item.people_count)">
											去加餐
										</view>
									...
									<template
										v-if="item.status==3 && (item.seat_info[0].seat_use_status==3 || item.seat_info[0].seat_use_status==2)">
										<view class="btm4" @click.stop="clearDesk(item.seat_id,item)">
											完成打扫
										</view>
									</template>
								</view>
							...
							<view class="" v-else>
								...
								<view class="content_Bom flex je ac" v-if="item.business_status==2">
									...
									<view class="btm4" @click.stop="cashPay(item.order_no,item)" v-if="item.status==1">
										现金支付
									</view>
									<view class="btm4" @click.stop="clearDesk(item.seat_id,item.seat_info[0])"
										v-if="item.status==3 && (item.seat_info[0].seat_use_status==3 || item.seat_info[0].seat_use_status==2)">
										完成打扫
									</view>
								</view>
		...
		<sjtabbar :ddShow='true' :pagesPath='pagesPath'></sjtabbar>
	</view>
</template>
	...
	export default {
		data() {
			return {
				...
				current: 0,
				poptext: "展开",
				pagesPath: '/pages/merchant/order/order',
				keyword: '',
				list1: [{
					name: '全部',
				}, {
		...
		methods: {
			...
			// 现金支付
			cashPay(order_no, item) {
				cash_payment({ order_no }).then(res => {
					if (res.code == 1) {
						item.status = 3
						this.page = 1;
						this.getlist()
